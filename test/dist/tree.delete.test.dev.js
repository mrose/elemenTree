...
describe("The delete method", function () {
  describe("for trees which have non-distinct node Ids", function () {
    test("returns false when the node id requested for deletion does not exist", function () {
    ...
    test("removes a node and its descendents", function () {
      var tree = _tree.Tree.factory({
        datum: {
          id: "root"
        },
      ...
      expect(tree.has(["a", "e", "f", "g", "h"])).toBe(false); // when inclusive is false, only descendents are deleted.
