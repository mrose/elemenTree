...
describe("The valuesOf method", function () {
  ...
  test("throws an error when depth is not an integer", function () {
    ...
    expect(function () {
      return tree.valuesOf(["a"], true, false, "depth");
    }).toThrow();
  });
  test("throws an error when depth is zero and inclusive is false", function () {
    ...
    expect(function () {
      return tree.valuesOf(["a"], false, false, 0);
    }).toThrow();
  ...
  describe("for trees which have non-distinct node Ids", function () {
    test("returns an empty values array when the path does not refer to an existing node", function () {
      var tree = _tree.Tree.factory({
        distinct: false
      });
    ...
    test("returns an empty values array when no descendents exist and inclusive is false", function () {
      var tree = _tree.Tree.factory({
        distinct: false
      });
    ...
    test("returns a single entry when no descendents exist and inclusive is true", function () {
      ...
      expect(tree.valuesOf(undefined, true)).toEqual([{
        id: "root"
      }]);
      expect(tree.valuesOf("root", true)).toEqual([{
        id: "root"
      }]);
      expect(tree.valuesOf(["root"], true)).toEqual([{
        id: "root"
      ...
      expect(tree.valuesOf("a", true)).toEqual([{
        id: "a"
      }]);
      expect(tree.valuesOf("a", true)).toEqual([{
        id: "a"
      ...
      expect(tree.valuesOf(["a", "b"], true)).toEqual([{
        id: "b"
      }]);
      expect(tree.valuesOf("a|b", true)).toEqual([{
        id: "b"
    ...
    test("returns a flat values array of descendents when nested is false", function () {
      var tree = _tree.Tree.factory({
        distinct: false
      });
    ...
    test("returns a nested [datum, values] array of descendents when nested is true", function () {
      var tree = _tree.Tree.factory({
        datum: {
          id: "root"
        },
  ...
  describe("for trees which have distinct node Ids", function () {
    ...
    test("returns an empty values array when no descendents exist and inclusive is false", function () {
      ...
      expect(tree.valuesOf("a")).toEqual([]);
      expect(tree.valuesOf("a")).toEqual([]);
      expect(tree.valuesOf(["a"])).toEqual([]); // we can use set with an ancestor because it's a distinct tree
