...
describe(`The set method`, () => {
  describe(`for trees which have distinct node Ids (default)`, () => {
    ...
    test(`appends a node`, () => {
      ...
      expect(tree.get('')).toEqual([['root'], { id: 'root', value: 0 }]);
      tree.clear();
      ...
      expect(rtrn).toEqual(['root']);
      expect(tree.get('root')).toEqual([['root'], { id: 'root', value: 0 }]);
      tree.clear();
      ...
      expect(tree.get(['a'])).toEqual([['root', 'a'], { id: 'a', value: 1 }]);
      tree.clear();
      ...
      expect(rtrn).toEqual(['root', 'a', 'b', 'c']);
      expect(tree.get(['a'])).toEqual([['root', 'a'], undefined]);
      expect(tree.get(['b'])).toEqual([['root', 'a', 'b'], undefined]);
      expect(tree.get(['c'])).toEqual([
        ['root', 'a', 'b', 'c'],
        { id: 'c', value: 3 },
      ]);
    ...
    test(`updates a node`, () => {
      ...
      expect(rtrn).toEqual(['root']);
      expect(tree.get('')).toEqual([['root'], { id: 'root', value: 10 }]);

      rtrn = tree.set([], { id: 'root', value: 20 });
      expect(rtrn).toEqual(['root']);
      expect(tree.get([])).toEqual([['root'], { id: 'root', value: 20 }]);

      rtrn = tree.set('root', { id: 'root', value: 30 });
      expect(rtrn).toEqual(['root']);
      expect(tree.get('root')).toEqual([['root'], { id: 'root', value: 30 }]);

      rtrn = tree.set(['root'], { id: 'root', value: 40 });
      expect(rtrn).toEqual(['root']);
      expect(tree.get(['root'])).toEqual([['root'], { id: 'root', value: 40 }]);
      tree.clear();
